...
# Cloud (1536) and local (768) embeddings live in separate collections
PROFILE = os.getenv("LLM_PROFILE", "cloud").lower()
if PROFILE == "local":
    COLLECTION_NAME = os.getenv("CHROMA_COLLECTION_LOCAL", "water_rag_local")
else:
    COLLECTION_NAME = os.getenv("CHROMA_COLLECTION_CLOUD", "water_rag")

print(f"Loading embedder (profile={PROFILE})...")
...
if existing is not None:
    ...
    print(f"   To re-ingest, delete the collection manually and run again.")
    print("\nExiting without making changes.")
    import sys

    sys.exit(0)
print(f"No existing collection '{COLLECTION_NAME}' found. Creating new one...")
